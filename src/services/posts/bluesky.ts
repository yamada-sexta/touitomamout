...
import { makeBlueskyPost } from "utils/bluesky/make-bluesky-post";
...
export class BlueskyPostSynchronizer implements PostSynchronizer {
  constructor(
    ...
    private tweeterHandle: TwitterHandle,
    private blueskyIdentifier: string,
  ) {}
  async syncPost(args: {
    tweet: Tweet;
    mediaList: Media[];
    ...
    for (const media of mediaList) {
      ...
      try {
        ...
        const res = await this.client.uploadBlob(blueskyBlob.blobData, {
          encoding: blueskyBlob.mimeType,
        });
        mediaAttachments.push({
          ...res,
          alt_text:
            media.type === "image" && media.photo.alt_text
              ? media.photo.alt_text
              : undefined,
    ...
    for (let i = 0; i < post.chunks.length; i++) {
      ...
      if (
        !Object.keys(quoteRecord).length &&
        !Object.keys(mediaRecord).length
      ) {
